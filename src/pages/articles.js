...
const FeaturedArticle = ({ img, title, time, summary, link }) => {
    return (
        ...
        <li className='relative col-span-1 w-full p-4 bg-light border-solid border border-dark dark:bg-dark dark:border-light  rounded-2xl'>
            ...
            <Link href={link} target="_blank"><h2 className='capitalize text-2xl font-bold my-2 mt-4 hover:underline'>{title}</h2></Link>
...
const Article = ({ img, title, date, link }) => {
    return (
        <motion.li
            ...
            <span className='text-primary dark:text-primaryDark font-semib pl-4'>{date}</span>
        </motion.li>
    )
}
